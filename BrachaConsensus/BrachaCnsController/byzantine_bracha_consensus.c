...
void malicious_bracha_broadcast(int originProcess, int value, int tag, int round, int dval);
...
bool condition(int round) {
    bool cond = false;
    if (roundsInfo[round].numValid >= N - T) {
        cond = true;
    }
    return cond;
}
...
void malicious_bracha_broadcast(int originProcess, int value, int tag, int round, int dval)
{
    ...
    printf("Process %d: Start Byzantine Bracha broadcast\n", processId);
    
    for (int i = 0; i < N; i++)
    {
        // Adjust depending on i, round etc to get the desired malicious behavior 
        int v = 0;
        int dv = 0;
        ...
        if (i == processId)
        {
            ...
            if (tag == 0) {
                ...
                brachaProcessMessages(originProcess, v, processId, tag, round, dv);
                continue;
            }
            
        }

        int message[7] = {round, originProcess, tag, processId, v, i, dv}; // format [round, origin, tag, from, value, to, dval] | tag 0:initial,1:echo,2:ready
        ...
        printf("Byzantine Process %d, round %d : Value %d sent to process %d with tag %d\n", processId, round, v, i, tag);
...
void brachaProcessMessages(int originProcess, int value, int fromProcess, int tag, int round, int dval)
{
    ...
    printf("Byzantine Process %d, round %d : Value %d from p%d Tag %d distinct count: %d\n", processId, round, 
    ...
    || countDistinctProcessesForValue(originProcess, value, 2, round) >= T+1 )) {
        printf("Byzantine Process %d, round %d : goes from step 0 to step 1 for value %d (origin p%d)\n", 
        processId, round, value, originProcess);
        roundsInfo[round].step[originProcess][value] = roundsInfo[round].step[originProcess][value] + 1;
        malicious_bracha_broadcast(originProcess, value, roundsInfo[round].step[originProcess][value], round, dval);
    }
    if (roundsInfo[round].step[originProcess][value] == 1 && ( countDistinctProcessesForValue(originProcess, value, 1, round) >= (N+T)/2 + 1 
    || countDistinctProcessesForValue(originProcess, value, 2, round) >= T+1 )) {
        printf("Byzantine Process %d, round %d : goes from step 1 to step 2 for value %d (origin p%d)\n", 
        processId, round, value, originProcess);
        roundsInfo[round].step[originProcess][value] = roundsInfo[round].step[originProcess][value] + 1;
        malicious_bracha_broadcast(originProcess, value, roundsInfo[round].step[originProcess][value], round, dval);
    }
    if (roundsInfo[round].step[originProcess][value] == 2 && countDistinctProcessesForValue(originProcess, value, 2, round) >= (2*T+1)) {
        printf("Byzantine Process %d, round %d : goes from step 2 to step 3 for value %d (origin p%d)\n", 
        processId, round, value, originProcess);
        roundsInfo[round].step[originProcess][value] = roundsInfo[round].step[originProcess][value] + 1;
        // Accept value
        roundsInfo[round].acceptedValue[originProcess] = value;
        printf("###############################\n");
        printf("Byzantine Process %d, round %d: Value %d accepted from process %d.\n", processId, round, value, originProcess);
        printf("###############################\n");
    ...
    while(1) {
        ...
        if (phase > 5) {
            ...
            while(1) {
                ...
                printf("Byzantine Process %d Round %d : Value %d (dval %d) received from process %d (origin %d) with tag %d in round %d\n", processId, 
        ...
        printf("Byzantine Process %d : Phase %d, \n", processId, phase);

        // #####################
        // 1st broadcast

        rnd = 3*phase + 1;
        printf("Byzantine Process %d : Round %d, \n", processId, rnd);

        // Broadcast the initial value
        
        value = initialValue;
        malicious_bracha_broadcast(processId, value, 0, rnd, dval);
        ...
        if (valueCount2[0] >= N/2 || valueCount2[1] >= N/2) {
            dval = 1;
        }
        ...
        if (decision != -1) {
            ...
            printf("###############################\n");
            printf("\n");
            printf("Byzantine Process %d: Value %d decided.\n", processId, decision);
            printf("\n");
            printf("###############################\n");
            printf("###############################\n");
        }
