...
// Format : [ type, from, to, msg, connfd, forkId, echo, numDelivered, [delivered] ]
typedef struct
{
  int type;   // send:0 or recv:1
  int round;    // round where the message was sent
  int originProcess;    // process that sent the message first
...
void put_msg_in_buffer(int index, int *receivedMessage)
{
  ...
  msgbuffer[index].originProcess = receivedMessage[2];
  msgbuffer[index].tag = receivedMessage[3];
  msgbuffer[index].from = receivedMessage[4];
  msgbuffer[index].to = receivedMessage[5];
  msgbuffer[index].msg = receivedMessage[6];
  msgbuffer[index].dval = receivedMessage[7];
  msgbuffer[index].connfd = -1;
  msgbuffer[index].forkId = receivedMessage[8];
  msgbuffer[index].echo = receivedMessage[9];
...
int initSocket(bool feedback)
{
  ...
  if (timeout)
  {
    ...
    tv.tv_usec = TIMEOUT_DURATION_MS; // 10000 microseconds 10000
...
void spawnProcesses()
{
  for (int i = 0; i < N; i++)
  {
    if ((processes[i] = fork()) == 0)
    {
      ...
      // ALGO CHG
      // Replace child process with bracha_consensus process
      setenv("LD_PRELOAD", "./redirect.so", 1);
      execl("./bracha_consensus", "bracha_consensus", processIdStr, initialValueStr, (char *)NULL);
      perror("execl failed");
...
void init()
{
  ...
  if (sem == SEM_FAILED)
  {
    if (errno == EEXIST)
    {
      ...
      if (sem_unlink("/sem_bracha_consensus") == -1)
      {
        perror("Error unlinking semaphore");
        exit(EXIT_FAILURE);
      }
      sem = sem_open("/sem_bracha_consensus", O_CREAT, 0644, 1);
      if (sem == SEM_FAILED)
      {
  ...
  if (sem_init_brd == SEM_FAILED)
  {
    if (errno == EEXIST)
    {
      // Semaphore already exists, try to unlink and create again
      printf("Semaphore already exists, trying to recreate it.\n");
      if (sem_unlink("/sem_bracha_consensus_init_brd") == -1)
      {
        perror("Error unlinking semaphore");
        exit(EXIT_FAILURE);
      }
      sem_init_brd = sem_open("/sem_bracha_consensus_init_brd", O_CREAT, 0644, 1);
      if (sem == SEM_FAILED)
      {
        perror("Error creating semaphore after unlinking");
        exit(EXIT_FAILURE);
    ...
    for (int r = 0; r < 3; r++) { // TODO ATTENTION ROUNDS START AT 1 ?? 
      for (int t = 0; t < 3; t++) {
        for (int op = 0; op < N; op++) {
          for (int v = 0; v < 2; v++)
          {
            if (state1[p][r][t][op][v] != state2[p][r][t][op][v])
            {
              return false;
            }
          }
  ...
  for (int r = 0; r < 3; r++) { // TODO ATTENTION ROUNDS START AT 1 ?? 
    for (int t = 0; t < 3; t++) {
      for (int op = 0; op < N; op++) {
        for (int v = 0; v < 2; v++)
        {
          if (processState1[r][t][op][v] != processState2[r][t][op][v])
          {
            return false;
          }
        }
...
bool checkAllStates()
{
  printf("\n[Controller] Check the validity of states :\n");
  bool invalid = false;
  for (int s = 0; s < numStates; s++)
  {
    if (systemStates[s].killed == 1)
    ...
    if (!checkStateValid(systemStates[s].valuesCount))
    {
      ...
      // TODO print messages might be a bit much, maybe log into file
    }
  }
  if (!invalid)
  {
...
{
  printf("msg:[type:%d, round:%d, originProcess:%d, tag:%d, from:%d, to:%d, value:%d, dval:%d, connfd:%d, forkId:%d, numDelivered:%d]\n",
         msgbuffer[index].type, msgbuffer[index].round, msgbuffer[index].originProcess, msgbuffer[index].tag, 
         msgbuffer[index].from, msgbuffer[index].to, msgbuffer[index].msg, msgbuffer[index].dval, msgbuffer[index].connfd,
    ...
    for (int r = 0; r < 3; r++) {
      for (int t = 0; t < 3; t++) {
        for (int op = 0; op < N; op++) {
          for (int v = 0; v < 2; v++)
          {
            systemStates[destState].valuesCount[p][r][t][op][v] = systemStates[originState].valuesCount[p][r][t][op][v];
          }
        }
      }
    }
  ...
  for (int r = 0; r < 3; r++) { // TODO ATTENTION ROUNDS START AT 1 ?? 
    for (int t = 0; t < 3; t++) {
      for (int op = 0; op < N; op++) {
        for (int v = 0; v < 2; v++)
        {
          systemStates[stateToUpdate].valuesCount[updatedProcess][r][t][op][v] = newProcessState[r][t][op][v];
        }
      }
    }
  }
...
int handleMessagePair(int recvIndex, int sendIndex, int fd, bool recv)
{
  ...
  kill(current_process, SIGSTOP); 
    ...
    {
     // ALGO CHG 
      int messageNoAction[INSTRUCTION_MESSAGE_SIZE] = {3, msgbuffer[sendIndex].round, msgbuffer[sendIndex].originProcess, msgbuffer[sendIndex].tag, msgbuffer[sendIndex].from, msgbuffer[sendIndex].msg, msgbuffer[sendIndex].dval, msgbuffer[sendIndex].to};
    ...
    int forkInfo[2];
    // ALGO CHG 
    int message[INSTRUCTION_MESSAGE_SIZE] = {1, msgbuffer[sendIndex].round, msgbuffer[sendIndex].originProcess, msgbuffer[sendIndex].tag, msgbuffer[sendIndex].from, msgbuffer[sendIndex].msg, msgbuffer[sendIndex].dval, msgbuffer[sendIndex].to};
    ...
    if (msgbuffer[sendIndex].from == 3 && msgbuffer[sendIndex].round == 2 && msgbuffer[sendIndex].tag == 2) // msgbuffer[sendIndex].from == 2  msgbuffer[sendIndex].from == 3
    {
      ...
      // ALGO CHG 
      int messageOp[INSTRUCTION_MESSAGE_SIZE] = {1, msgbuffer[sendIndex].round, msgbuffer[sendIndex].originProcess, msgbuffer[sendIndex].tag, msgbuffer[sendIndex].from, opValue, msgbuffer[sendIndex].dval, msgbuffer[sendIndex].to};
      ...
      if (compareProcessState(newProcessState, newProcessStateOp))
      {
        ...
        // ALGO CHG 
        int killMessage[INSTRUCTION_MESSAGE_SIZE] = {2, -1, -1, -1, -1, -1, -1, -1};
...
int main()
{
  ...
  sem_close(sem);
  sem_unlink("/sem_bracha_consensus"); 

  sem_close(sem_init_brd);
  sem_unlink("/sem_bracha_consensus_init_brd");
