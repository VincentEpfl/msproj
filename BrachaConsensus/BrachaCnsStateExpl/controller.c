...
int handleMessagePair(int recvIndex, int sendIndex, int fd, bool recv)
{
  ...
  // if (canDeliver(statesToUpdate, numStatesToUpdate, j, i))
  {
    ...
    */
    ...
    if (msgbuffer[sendIndex].from == 3 && msgbuffer[sendIndex].tag == 2 && msgbuffer[sendIndex].round == 2) // msgbuffer[sendIndex].from == 2  msgbuffer[sendIndex].from == 3
    {
      ...
      if (compareProcessState(newProcessState, newProcessStateOp))
      {
        ...
        // ALGO CHG 
        int killMessage[8] = {2, -1, -1, -1, -1, -1, -1, -1};
