...
typedef struct 
{
// format (est/aux, round, source, val, destination)
int type;
int roundNum;
int senderId;
int receivedValue;
...
void broadcast(int value, int roundNumber) {
    ...
    printf("Process %d: Start byzantine broadcast\n", processId);
    
    for (int i = 0; i < N; i++)
    {
        // This is a byzantine process, so we hardcode the values it sends independantly 
        ...
        if (i == processId)
        {
            ...
            printf("Process %d, Round %d : Byz Value %d sent to myself with tag AUX\n", processId, rnd, byz_val);
            roundsInfo[roundNumber].received_values_aux[processId][byz_val] = 1;
            continue;
        }

        int message[5] = {0, rnd, processId, byz_val, i}; // format (est/aux, round, source, val, destination)
        ...
        printf("Process %d, Round %d : Byz Value %d sent to process %d with tag AUX\n", processId, rnd, byz_val, i);
        close(sockfd);
    }
}
...
void BV_broadcast(int value, int roundNumber) 
{
    ...
    printf("Process %d: Start Byzantine BV broadcast\n", processId);
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            // Send value to itself + receive
            printf("Process %d, Round %d : Byz Value %d sent to myself with tag EST\n", processId, rnd, byz_val);
            roundsInfo[roundNumber].received_values[processId][byz_val] = 1;
        ...
        int message[5] = {1, rnd, processId, byz_val, i}; // format (est/aux, round, source, val, destination)
        ...
        printf("Process %d, Round %d : Byz Value %d sent to process %d with tag EST\n", processId, rnd, byz_val, i);
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        printf("Byzantine Process %d : Round %d, est = %d\n", processId, rnd, est);
        ...
        while (roundsInfo[rnd].committedValues[0] == 0 && roundsInfo[rnd].committedValues[1] == 0)
        {
            ...
            printf("Byzantine Process %d: Value %d received from process %d, round %d, type %d (1=EST)\n", processId, receivedValue, senderId, r, type);
            if (receivedValue >= 0)
        ...
        if (values[0] && values[1]) {
            ...
            if (!decided) {
                ...
                decided = true;
                printf("#############################################\n");
                printf("Byzantine Process %d decides on value %d\n", processId, s);
                printf("#############################################\n");
            }
            est = s;
        } else {
